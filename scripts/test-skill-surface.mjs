...
assert.match(skill, /call cycle_connection_status/);
...
const result = await run(["call", "cycle_connection_status", "--json", "{}"]);
...
console.log(JSON.stringify({ ok: true, suite: "skill-surface", tool: "cycle_connection_status", files: pkg.files }, null, 2));
